...
public class ApiWorlScheduleController {
    ...
    @DeleteMapping("/secure/workschedules/delele/{wsId}")
    public ResponseEntity<?> delete (@PathVariable(value="wsId") int id, Principal principal){
        String username = principal.getName();
        User user = this.userDetailService.getUserByUsername(username);
        Workschedule ws = this.workScheduleService.getWorksScheduleById(id);
        try {
            if (Objects.equals(user.getId(), ws.getDoctorId().getUser().getId())){
                this.workScheduleService.deleteWorkSchedule(id);
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }
            else{
                return new ResponseEntity<>("Không thể xóa do không phải chủ sở hữu!", HttpStatus.FORBIDDEN);
            }
